...
HVG = pd.read_csv(r"C:\Users\Breanne Sparta\Desktop\from posieden\DDG_paper\zheng5k_gene_sets\zheng5k_HVGs.csv", index_col=0, nrows = 1) #remoe the extra indeces that come up from the duplicate geneta\Desktop\from posieden\DDG_paper\zheng5k_gene_sets
#HVG = pd.read_csv(r"/home/breanne/sourcedata/Zheng9/zheng5k_HVGs.csv", index_col=0,skiprows=[1156]) #remoe the extra indeces that come up from the duplicate gene
...
title_list = ["HVGs"]
gene_list = [HVG.columns]

#group confusion groups into the same index
facs_headers = np.array(HVG.index)  # headers h is index, so data is cell x gene
...
total_data = HVG.values
total_data= pd.DataFrame(total_data, index = facs_headers, columns = HVG.columns)
...
for data_group, cname in zip(data_list,countname):
    ...
    for glabel, gname in zip(gene_list,title_list):
        ...
        total_data = total_data.fillna(0) #there shouldnt be any though

        data = sc.AnnData(total_data)
        ...
        ax = plt.gca()
        plt.savefig(large_root + "/" + "HVGs_cpm_facs_tsne_plot.eps")
        plt.savefig(large_root + "/" + "HVGs_cpm_facs_tsne_plot.pdf")

        sc.tl.umap(data)
        ax = plt.gca()
        plt.savefig(large_root + "/" + "HVGs_cpm_facs_umap_plot.eps")
        plt.savefig(large_root + "/" + "HVGs_cpm_facs_umap_plot.pdf")
