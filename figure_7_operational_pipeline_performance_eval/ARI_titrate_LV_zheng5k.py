...
large_root = "/media/timothyhamilton/data1/Tim_Hamilton/Zheng_FACS_pbmcs/zheng9_5k/ARI_titration" #make ARI folder
label = 'zheng5k_mergTls'

#usec = list(range(0,101))
total_data = pd.read_csv("/media/timothyhamilton/data1/Tim_Hamilton/Zheng_FACS_pbmcs/zheng9_5k/zheng5k_dropzeros.csv", index_col=0) #gene x cells use cols
cpm_data = pd.read_csv("/media/timothyhamilton/data1/Tim_Hamilton/Zheng_FACS_pbmcs/zheng9_5k/zheng5k_cpmlog.csv", index_col=0) #cell x genes? use rows
...
HVG = pd.read_csv("/media/timothyhamilton/data1/Tim_Hamilton/Zheng_FACS_pbmcs/zheng9_5k/zheng5k_HVGs.csv", index_col=0,skiprows=[1156],usecols = [0,1]) #remoe the extra indeces that come up from the duplicate gene
...
total_data= pd.DataFrame(total_data, index = facs_headers, columns = gene_list[7])
cpm_data = cpm_data.values
cpm_data= pd.DataFrame(cpm_data, index = facs_headers, columns = gene_list[7])
eq_data = total_data.values
eq_data = equalizer(eq_data)
eq_data = pd.DataFrame(eq_data, index = facs_headers, columns = gene_list[7])
