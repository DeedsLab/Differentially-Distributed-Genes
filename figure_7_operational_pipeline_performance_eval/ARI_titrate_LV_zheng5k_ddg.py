...
WC_set= WC_set + marker_set
DDG_set= DDG_set + marker_set
M3Drop_set= M3Drop_set + marker_set
Townes_set= Townes_set + marker_set
HVG_set = HVG_set + marker_set
nbdrop_set= nbdrop_set + marker_set
nbdisp_set = nbdisp_set + marker_set
...
genenames = np.array(total_data.columns)
data = total_data.to_numpy(dtype=int)
data = pd.DataFrame(data=data, index=barcodes, columns=genenames)
adata = sc.AnnData(data)
sc.pp.normalize_total(adata, target_sum=1e4)
sc.pp.log1p(adata)
cpm_data = pd.DataFrame(data=adata.X, index=adata.obs_names, columns=adata.var_names)
print(cpm_data.head())
...
cpm_data= pd.DataFrame(cpm_data, index = facs_headers, columns = genenames)
