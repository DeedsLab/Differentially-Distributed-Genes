...
WC = pd.read_csv(r"/home/breanne/ddgs/zheng5k_oldWilcox_bf8k.csv", index_col=0) #c x g#HVG = pd.read_csv(r"/home/breanne/sourcedata/Zheng9/zheng5k_HVGsh.csv", index_col=0,skiprows=[1156],usecols = [0,1]) #remoe the extra indeces that come up from the duplicate gene
...
facs_headers = np.array(WC.index)  # headers h is index, so data is cell x gene
...
WC = WC.values
WC= pd.DataFrame(WC, index = facs_headers, columns = WC_set)
