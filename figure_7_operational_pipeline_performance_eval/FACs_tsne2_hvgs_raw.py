...
total_data= pd.DataFrame(total_data, index = facs_headers, columns = gene_list[0])
#cpm_data = cpm_data.values
#cpm_data= pd.DataFrame(cpm_data, index = facs_headers, columns = gene_list[0])
...
for data_group, cname in zip(data_list,countname):
    ...
    for glabel, gname in zip(gene_list,title_list):
        ...
        sc.pl.tsne(data, color=['facs'], save="HVGs_raw_facs_tsne_plot.eps")
