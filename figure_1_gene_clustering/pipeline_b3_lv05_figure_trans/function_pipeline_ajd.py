...
def ajd_generate(cluster_size,root):
	rootpath = root
	print("well, good luck, :)")
	amtclusters = (len([name for name in os.listdir(rootpath+"/subclusters/") if os.path.isfile(os.path.join(rootpath+"/subclusters/",name)) and "asbasisonly" not in name]))
	print("there are" + str(amtclusters) + "clusters")
	frame_list=[]
	neighborhoods=[]
	sub_shape=[]
	nameit=[]
	for j in range(0,amtclusters):
		alreadymadeclusters = (sorted([name for name in os.listdir(rootpath+"/subclusters/") if os.path.isfile(os.path.join(rootpath+"/subclusters/",name)) and "asbasisonly" in name]))
		if "subcluster_"+str(j)+"asbasisonly.csv" in alreadymadeclusters:
				print("already made ")
				print("moving on")
		else:
			print("performing operations to create"+"subcluster_"+str(j)+"asbasisonly.csv")
			embedded_data = pd.read_csv(rootpath+"/"+"subclusters/"+"subcluster_"+str(j)+".csv")
			...
			nameit.append(rootpath+"/"+"subclusters/"+"subcluster_"+str(j)+"asbasisonly.csv")
