...
print("Reading Data")
new_frame = pd.read_csv(large_root + "/planarian.csv", index_col = 0)
new_frame = new_frame.T
dropzero = new_frame[np.all(new_frame == 0, axis=1)].index
newframe = new_frame.drop(dropzero, 'index') #drop genes that are not detected in any cells
#new_frame.to_csv("/home/breanne/sourcedata/planarian/planarian_dropzeros.csv")
clust_list = [newframe]
label = ['planarian']
