...
large_root = r"/home/breannesparta/ddgs/zhengnine"
...
for rep in rep_list:
    for int in intlist:
        ...
        #typelist = ['bcells', 'thelp', 'cd34', 'cd56_nk', 'cytotoxic_t', 'memory_t', 'monocytes', 'naive_cytotoxic', 'naive_t', 'regt']
        comb = pd.concat([bcells, thelp, cd56_nk, cytotoxic_t, memory_t, monocytes, naive_cytotoxic, naive_t, regt])
        comb = comb.fillna(0)
        comb = comb.loc[:, (comb != 0).any()]  # drop genes that are not detected in any cell
        int2 = str(int)
        comb.to_csv(r"/media/timothyhamilton/Seagate Desktop Drive1/breanne/ddgs/zheng_resampled/" + int2 + "_rep_" + rep + "zheng9_resamp.csv")
        comb = comb.T

        label = [int2 + "_rep_" + rep + "zheng_resamp"]
        clust_list = [comb]
        ...
        for d, l in zip(clust_list, label):
            ...
            #getparams.to_csv(large_root + "/gene_params/" + l + "gene_params.csv", header=None, index=None)
