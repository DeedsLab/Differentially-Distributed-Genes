...
DDG20 = pd.read_csv(large_root + "/cellgene/zheng9pcap20DDG.csv", index_col=0, nrows=1) #c x g
DDG80 = pd.read_csv(large_root + "/cellgene/zheng9pcap80DDG.csv", index_col=0, nrows=1) #c x g
...
#hvgs2k= HVG2.columns.tolist()
hvgs8k= rename(HVG8.columns.tolist())
#ddgsp1= DDG1.columns.tolist()
ddgsp20= rename(DDG20.columns.tolist())
ddgsp80= rename(DDG80.columns.tolist())

#HVG_set = [x for x in HVG_first if x in all_set]

#gene in second position will be reference list
title_list = ["all_genes","8k_hvgs","pcap20_DDGs","pcap80_DDGs"]
gene_list = [all_set, hvgs8k, ddgsp20, ddgsp80]
...
for cell_group, clabel in zip(data_list,cellname):
    ...
    for gene_group, glabel in zip(gene_list2, title_list2):
        ...
        venn2([set(gene_list[1]), set(gene_group)], set_labels=(title_list2[1], glabel))
        ax.set_rasterized(True)
        plt.savefig(large_root + "/" + label + clabel + "gene_overlap_HVG8k" + glabel + ".png")
        plt.savefig(large_root + "/" + label + clabel + "gene_overlap_HVG8k" + glabel + ".eps")
        fig.clear()
        ...
        print(stat_frame_all)
        print(glabel)
        stat_frame_gg = stat_frame_all.loc[test_group, :] #breaks here for just this one  #why is none of test group in statframe all

        #intersect w test group in this case hvg 8k
        ddg_group = pd.Series(list(gene_list[1])) #hvg8k
        stat_frame_ddgs = stat_frame_all.loc[ddg_group,:]

        set_2 = frozenset(gene_list[1])
        intersectWC = [x for x in gene_group if x in set_2] #hvg8k
        i = len(intersectWC)
        print("intersect"+str(i))
        intersectWC = list(intersectWC)
        stat_frame_int = stat_frame_all.filter(intersectWC, axis=0) #intersect w hvg8k
        ...
        ax = fig.add_subplot(111)
        ax.set_title(label + clabel + glabel + "overlap with hvg8k set", fontsize=fs)
        ...
        ax.legend()
        plt.tight_layout()
        plt.savefig(large_root + "/" + label + glabel + clabel + "scatter_set_intersect_hvg8k_dispersion.png")
        plt.savefig(large_root + "/" + label + glabel + clabel + "scatter_set_intersect_hvg8k_dispersion.eps")
        fig.clear()
