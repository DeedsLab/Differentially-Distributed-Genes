...
title_list = ["all_genes","WC","DDGs","M3Drop","Townes","NBdrop","NBdisp","HVGs"]
gene_list = [all_set, WC_set, DDG_set, M3Drop_set, Townes_set,nbdrop_set, nbdisp_set,HVG_set]
...
for cell_group, clabel in zip(data_list,cellname):
    ...
    for gene_group, glabel in zip(gene_list2, title_list2):
        ...
        sc1 = plt.plot(ma, Yexp, linewidth=3)
        plt.tight_layout()
        plt.savefig(large_root + "/" + label + glabel + clabel+ "scatter_set_intersect_WC.png")
        plt.savefig(large_root + "/" + label + glabel + clabel+ "scatter_set_intersect_WC.eps")
        fig.clear()
